    ...
    : ICachedCourseService
{
        ...
        var dto = await cache.GetOrCreateAsync(
            ...
            async (state, token) =>
            {
                ...
                logger.LogInformation(
                    "Cache MISS for {Key} fetching from DB", key);
    ...
    {
        Console.WriteLine("CachedCourseService.GetAllCoursesAsync called");
    ...
    {
        logger.LogInformation(
            "Invalidating cache tag {Tag}", CacheKeys.CoursesTag);

        await cache.RemoveByTagAsync(CacheKeys.CoursesTag, ct);
    }

    Task<CourseDto?> ICachedCourseService.UpdateAsync(int id, UpdateCourseRequest request, CancellationToken ct)
    {
